...
async function getAsaUsdPrice(asaId) {
  ...
  // 4) Fallback: Vestige price aggregator
  try {
    const vestigeRes = await axios.get(
      `https://free-api.vestige.fi/asset/${asaId}/price`,
      { timeout: 10000 }
    );
    const vPrice = parseFloat(vestigeRes.data?.price ?? '0');
    if (vPrice > 0) {
      setCache(cacheKey, vPrice);
      return vPrice;
    }
  } catch {
    // try next fallback
  ...
  // 4b) Fallback: Pact.fi price
  try {
    const pactRes = await axios.get(
      `https://api.pact.fi/api/assets/${asaId}`,
      { timeout: 10000 }
    );
    const pPrice = parseFloat(pactRes.data?.price ?? '0');
    if (pPrice > 0) {
      setCache(cacheKey, pPrice);
      return pPrice;
    }
  } catch {
    // try manual override below
  }

  // 5) Final fallback: manual price override from feeConfig
  try {
    ...
    if (override && override > 0) {
      logger.warn(`getAsaUsdPrice(${asaId}): using manual override $${override}`);
