...
const PStakebanner = () => {
  ...
  const fetchStats = async () => {
    try {
      ...
      // Compute USD TVL: sum each pool's totalAmountStaked * token price
      let totalTvl = 0
      try {
        const asaIds = [...new Set(allPools.map((p: any) => Number(p.stakeToken?.id)).filter((id: number) => !isNaN(id) && id > 0))] as number[]
        const priceMap = await fetchPriceMap(asaIds)
        totalTvl = allPools.reduce((sum: number, p: any) => {
          const price = priceMap[p.stakeToken?.id?.toString()] ?? 0
          return sum + (p.totalAmountStaked || 0) * price
        }, 0)
      } catch {
        totalTvl = 0
      }
